...
function params(){
	...
	Dialog.addNumber("Correction Factor (1+% underestimation of hand-counted length)",1.12);
...
function doit(stack){
	...
	saveAs("Tiff", par1+"\\Vessel Macro Output\\"+rawName+"_mask");
	...
	saveAs("Tiff", par1+"\\Vessel Macro Output\\"+rawName+"_skeleton1");
	selectWindow(rawName+"_skeleton1.tif");
	capnum=vesselanalysis(states[14],20,2);
	runInSquares(40,0,1,0,5,180);
	tortuosity=vesselanalysis(states[11],states[14],1);
	roiManager("Deselect");
	roiManager("Delete");
	close(name);
	...
	run("Set Scale...", "distance=1 known="+pixelWidth+" unit=um");
	selectWindow(rawName+"_skeleton1.tif");
	avelengths=vesselanalysis(states[11],0,0);
	close(rawName+"_skeleton1.tif");
	close(name);
	close("Result of MAX_A-0001");
	close("Mask of Result of MAX_A-0001");
	close("MAX_Result of MAX_A-0001");
	open(par1+"\\Vessel Macro Output\\FWHM_stack.tif");
	rename("stack");
	nROIs = roiManager("count");
	open(par1+"\\Vessel Macro Output\\FWHMROI.roi");
	...
	totalvessellength=length*states[12]*pixelWidth;	
	totalvessellength=totalvessellength/1000000;
	roivolume=(numslices*states[1]*roiarea)/1000000000;
	density=totalvessellength/roivolume;
	perarea=varea1/roiarea1;
	pervol=vvol/roivolume;
	myTableAll(rawName,density,roivolume,totalvessellength,avelengths[0],pervol,perarea,width,tortuosity[0],tortuosity[1],slice,states[3],states[2],roiarea1,varea1,vvol,capnum,FWHM,allBeads,restrictedBeads);
	
	fileName=Array.concat(fileName,rawName);
	fileDensity=Array.concat(fileDensity,density);
	fileROIVolume=Array.concat(fileROIVolume,roivolume);
	fileVesselLength=Array.concat(fileVesselLength,totalvessellength);
	fileAverageLength=Array.concat(fileAverageLength,avelengths[0]);
	filePercentVolume=Array.concat(filePercentVolume,pervol);
	filePercentArea=Array.concat(filePercentArea,perarea);
	fileAverageWidth=Array.concat(fileAverageWidth,width);
	fileMeanTortuosity=Array.concat(fileMeanTortuosity,tortuosity[0]);
	fileMedianTortuosity=Array.concat(fileMedianTortuosity,tortuosity[1]);
	fileNumberSlices=Array.concat(fileNumberSlices,slice);
	fileSlicesSampled=Array.concat(fileSlicesSampled,states[3]);
	fileTissueThickness=Array.concat(fileTissueThickness,states[2]);
	fileROIArea=Array.concat(fileROIArea,roiarea1);
	fileVesselArea=Array.concat(fileVesselArea,varea1);
	fileVesselVolume=Array.concat(fileVesselVolume,vvol);
	fileCapillaryNumber=Array.concat(fileCapillaryNumber,capnum);
	fileSignalRatio=Array.concat(fileSignalRatio,rat);
	fileFWHM=Array.concat(fileFWHM,FWHM);
	fileAllBeads=Array.concat(fileAllBeads,allBeads);
	fileRestrictedBeads=Array.concat(fileRestrictedBeads,restrictedBeads);
	junctioncount2=0;
	roiManager("Deselect");
	roiManager("Delete");
	open(dir+name);
...
if (states[13]==1){
	directName=File.getName(dir);
	selectWindow("Results");
	run("Set Measurements...", "  redirect=None decimal=9"); 
	run("Close");
	for (i=0; i<lengthOf(fileName);i++)
	{
	setResult("File", i,fileName[i]);
	setResult("Vessel Density (m/mm3)", i,fileDensity[i]);
	setResult("ROI Volume (mm3)", i,fileROIVolume[i]);
	setResult("Total All Vessel Length (m)", i,fileVesselLength[i]);
	setResult("Average vessel length (um)", i,fileAverageLength[i]);
	setResult("%Volume", i,filePercentVolume[i]);
	setResult("%Area", i,filePercentArea[i]);
	setResult("Average Width (um)", i,fileAverageWidth[i]);
	setResult("Mean Tortuosity (>"+states[14]+"um)", i,fileMeanTortuosity[i]);
	setResult("Median Tortuosity (>"+states[14]+"um)", i,fileMedianTortuosity[i]);
	setResult("Total # Slices", i,fileNumberSlices[i]);
	setResult("# Sampled Slices", i,fileSlicesSampled[i]);
	setResult("Tissue Thickness (um)", i,fileTissueThickness[i]);
	setResult("ROI Area (mm2)", i,fileROIArea[i]);
	setResult("Vessel Area (mm2)", i,fileVesselArea[i]);
	setResult("Vessel Volume (mm3)", i,fileVesselVolume[i]);
	setResult("Capillary number", i,fileCapillaryNumber[i]);
	setResult("Z Axis FWHM", i,fileFWHM[i]);
	setResult("All Maxima",i,fileAllBeads[i]);
	setResult("Maxima in Projection",i,fileRestrictedBeads[i]);
	
	}
	if (states[5]==1){
		saveAs("Results", par1+"\\Vessel Macro Output\\"+directName+"_Vessel Density Table");
	}
	else{
		saveAs("Results", par1+"\\Vessel Macro Output\\"+rawName+"_Vessel Density Table");
	}
}
